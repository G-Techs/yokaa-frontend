    ...
    >
      <div>
        <div className="px-3 py-5 flex justify-between mobile:items-center">
          <h1 className={`text-2xl font-bold ${!isMouseOverLeftNav && "text-center"} mobile:text-left`}>Yokaa</h1>
          <button onClick={() => setIsMenuOpened(!isMenuOpened)} className="hidden mobile:block">
            <CloseIcon />
          </button>
          <button onClick={onPinLeftNav} className={`${isMouseOverLeftNav ? "block" : "hidden"} ${isLeftNavPined && "p-2 rounded-full text-white bg-primary"} mobile:hidden`}>
            <PinIcon />
          </button>
        </div>
        <NavContentIcons isMouseOverLeftNav={isMouseOverLeftNav} />
        <div className={`${!isMouseOverLeftNav ? "w-0 overflow-hidden" : "w-full"} mobile:w-full`}>
          <DashBord isDashboardSubmenu={isDashboardSubmenu} toggleSubmenu={toggleSubmenu} />
          <AdminMenu isAdminSubmenu={isAdminSubmenu} toggleSubmenu={toggleSubmenu} />
        </div>
      </div>
      <button onClick={toggleTheme} className="flex justify-center items-center bg-primary m-6 py-3 rounded-full text-white font-Mulish font-bold">
        {isDarkMode ? <MoonIcon /> : <SunIcon />}
        <span className={`px-2 ${!isMouseOverLeftNav && "hidden"} mobile:block`}>{isDarkMode ? "Dark" : "Light"}</span>
      </button>
    </div>
  );
};

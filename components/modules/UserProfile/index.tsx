...
const UserProfile: FC<{ isTopNav: boolean; dailySales: typeof dailySalesData }> = ({ isTopNav, dailySales }) => {
  return (
    <div className={`mt-24 h-auto flex justify-center md:gap-4 mobilesm:flex-col mobile:mr-5 mobile:pb-40 transition-all mr-5 ${isTopNav && "mobile:mt-40 transition-all"}`}>
      <div className="flex flex-col  w-1/3 my-5 bg-white  px-5 py-5 rounded-xl shadow-xl  items-left mobilesm:w-full  items-center">
        <img src="https://templates.iqonic.design/muzik/html/images/user/1.jpg" alt="user" className="w-24 h-24s rounded-full mb-8" />
        ...
        <p className="text-center text-xs font-light text-gray-600">
          Phasellus faucibus mollis pharetra. Proin blandit ac massa.Morbi nulla dolor, ornare at commodo non, feugiat non nisi
        </p>
            ...
            return (
              <div className="flex justify-between pt-5" key={items.id}>
                <div className="flex gap-4 ">
                  <img src={items.img} alt="ds" className="w-12 h-12 rounded-full" />

...
import { PauseIcon, PlayCircleIcon } from "../Vectors";
    ...
    <div className="py-4">
      <div className="flex flex-wrap -ml-4 -mr-4">
        {props.data.map((song: SongoData) => {
          ...
          return (
            <div key={id} className="w-full md:w-1/3  lg:w-1/6 " onMouseOver={() => setIsHovering(id)} onMouseLeave={() => setIsHovering(-1)}>
              <div className="m-4 border rounded-bl-2xl rounded-tr-2xl h-50">
                <div className="flex flex-col justify-center items-center">
                  <img src={img} alt={artist} className="rounded-bl-2xl w-full rounded-tr-2xl h-full md:h-44 object-cover" />
                  <div
                    className={`absolute ${isHovering === id || isPlayingIndex === id ? "block" : "hidden"} `}
                    onClick={() => {

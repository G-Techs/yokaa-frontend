...
import { LoopIcon, PlayListIcon, ShuffleIcon, VNext, VPlay, VPrev, VolumeDownIcon, VolumeUpIcon } from "../modules/Vectors";
  ...
  return (
    <div
      className={`z-20 w-full ${
        isFixed ? "fixed justify-between bg-gray-700 " : "absolute justify-center items-center"
      } bottom-0 pb-2 pt-3 px-6 md:px-8 flex flex-col md:flex-row items-center h-28 md:h-20 opacity-90`}
    >
      <div className={`${isFixed ? "w-full md:w-3/6 lg:w-2/6 flex box" : "hidden"}`}>
        {isFixed && (
      ...
      <div className={`${isFixed ? "md:3/6 lg-4/6 mg:gap-6 justify-between" : "w-full justify-center"} flex flex-col md:flex-row items-center`}>
        <div className="flex justify-center items-center gap-2">
          <span className="text-gray-200 text-base font-medium">00:00</span>
          <Slider className={`${isFixed ? "w-60 md:w-28 lg:w-40" : "w-20"} mx-1 text-primary`} />
          <span className="text-gray-200 text-base font-medium">00:00</span>
        </div>
        <div className="hidden md:flex items-center justify-center gap-3 lg:gap-4 w-1/3">
          <ShuffleIcon className={`hidden w-6 h-6 text-primary cursor-pointer ${!isFixed ? "hidden" : "lg:inline"}`} />
          ...
          <LoopIcon className={`hidden w-6 h-6 text-primary cursor-pointer ${!isFixed ? "hidden" : "lg:inline"}`} />
        </div>
        <div className="hidden lg:flex items-center justify-center gap-2 w-1/3 ">
          <PlayListIcon className={`w-6 h-6 text-gray-200 ${!isFixed && "hidden"}`} />
